...
void	Channel::_setUserLimitChannel(std::istringstream & iss, Client const & client)
{
	...
	for (size_t i = 0; i < limitStr.length(); i++)
	{
		...
		{
			_sendMessageToClient(client, "The limit must be numeric\n");
			return ;
		}
	}
	iss >> limitStr;
	if (iss.eof())
		_sendMessageToClient(client, "Usage: MODE <#channel> [+l] <limit>\n");
	else
	{
		iss >> limitStr;
		if (iss.eof() == false)
		...
		else
		{
			...
			if (limitInt > 50)
				_sendMessageToClient(client, "The limit cannot exceed 50\n");
			else
			{
				_modes |= 1 << USER_LIMIT;
				_userLimit = limitInt; 
